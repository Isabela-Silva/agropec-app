...
import { FilterType, MapFeature, MapViewProps } from '../types';
...
const MapController: React.FC = () => {
  ...
  useEffect(() => {
    ...
    mapData.features.forEach((feature: MapFeature) => {
...
const convertCoordinates = (coordinates: number[][][]): L.LatLngExpression[][] => {
  return coordinates.map((ring) => ring.map(([x, y]) => [y, x] as L.LatLngExpression));
};
  ...
  return (
    ...
    >
              ...
              >
                <Popup>
                  <div className="text-sm">
                    <h3 className="mb-1 text-base font-bold">{feature.properties.nome}</h3>
                    <p className="italic text-gray-600">{feature.properties.tipo}</p>
                    {feature.properties.localização && (
                      <p className="mt-1 text-gray-500">Local: {feature.properties.localização}</p>
                    )}
                  </div>
                </Popup>
              </Polygon>
      ...
      {(activeFilter || searchTerm) && (
        <div className="absolute right-4 top-32 z-[1000]">
          <div className="floating-element rounded-full border border-gray-200 bg-white px-3 py-2 shadow-lg">
            <span className="text-xs font-medium text-gray-600">{filteredFeatures.length} local(ais)</span>
          </div>
        </div>
      )}
    </div>
